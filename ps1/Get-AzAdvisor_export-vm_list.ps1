...
$location = (Get-AzAdvisorRecommendation -Category Cost | Where-Object {$_.impactedfield -eq "Microsoft.ReservedInstances/reservedInstances"} | ForEach-Object {$_.ExtendedProperties.values.split(",")[0]} ) |Sort-Object |Get-Unique


#collect the VMSIZE of the Azure recommended hardware types: and select unique entries.
$vmsize = (Get-AzAdvisorRecommendation -Category Cost | Where-Object {$_.impactedfield -eq "Microsoft.ReservedInstances/reservedInstances"} | ForEach-Object {$_.ExtendedProperties.Values.split(",")[1]}) | Sort-Object |Get-Unique
